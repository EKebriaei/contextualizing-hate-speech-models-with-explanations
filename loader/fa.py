...
class FaProcessor(DataProcessor):
    """
    ...
    def _create_examples(self, data_dir, split, label=None):
        ...
        :return:
        """
        addr = os.path.join(data_dir, '%s.xlsx' % split)
        data = pd.read_excel(addr)
        examples = []
        for i, row in data.iterrows():
            example = InputExample(text_a=row["tweet"], guid='%s-%s' % (split, i))
            label = int(row["class"])
            example.label = label
